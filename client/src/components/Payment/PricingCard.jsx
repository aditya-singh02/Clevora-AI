  ...
  return (
    ...
    >
      ...
      <div className="mb-6">
        ...
        >
          <RiCoinLine size={14} className="text-amber-400 animate-pulse" />
      ...
      <div className="space-y-3 mb-8 flex-1">
        {plan?.features?.map((f) => (
          <div
            key={f}
            className="flex items-center gap-2.5 text-sm text-slate-400 font-medium"
          >
            <BsCheckCircleFill
              size={12}
      ...
      {/* CTA Button with State Guards */}
      <motion.button
        onClick={onCTA}
        disabled={isGlobalLoading || disabled} // Disable button during loading or if explicitly disabled
        whileHover={{ scale: isGlobalLoading ? 1 : 1.02 }}
        whileTap={{ scale: isGlobalLoading ? 1 : 0.97 }}
